...
新特性和问题修复须在 [`CHANGELOG.md`](CHANGELOG.md) 中记录日期和简要内容；涉及使用约束、数据流或验收方式的变更，同时更新本开发文档。
...
- 导出状态下简历容器使用 `overflow: hidden`，打印介质下 `html` 和 `body` 隐藏滚动；编辑状态仍保留滚动能力。
...
截至本文更新时间，共有 4 个测试套件、9 个测试。它们覆盖应用挂载、导入布局校验、图片双模式持久化、主题色切换和 PDF 导出滚动条行为。
